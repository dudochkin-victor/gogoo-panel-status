...
mps_geotag_pane_ensure_marker_visible (MpsGeotagPane *pane)
{
	...
	ChamplainMarker *markers[2] = { NULL, };

	markers[0] = priv->your_location_marker;
	markers[1] = NULL;
                               ...
                               gdouble        longitude)
{
	...
	if (!priv->your_location_marker)
	{
		...
		priv->your_location_marker = champlain_label_new_with_text (_("Your location"),
		                                                            ...
		                                                            &text_color,
		                                                            &marker_color);
		champlain_marker_layer_add_marker (priv->markers_layer, priv->your_location_marker);

	}

	champlain_location_set_location (priv->your_location_marker,
	                                 latitude,
	                                 longitude);
                                   ...
                                   MpsGeotagPane *pane)
{
	...
	priv->latitude = champlain_view_y_to_latitude (priv->map_view, (gdouble) (*bevent).y);
	priv->longitude = champlain_view_x_to_longitude (priv->map_view, (gdouble) (*bevent).x);
	priv->position_set = TRUE;
	mps_geotag_pane_update_marker (pane, latitude, longitude);
	mx_button_set_toggled (MX_BUTTON (priv->guess_location_button), FALSE);
	return TRUE;
